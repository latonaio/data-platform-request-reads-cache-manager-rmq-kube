package dpfm_api_input_reader

import (
	"encoding/json"
	"fmt"

	rabbitmq "github.com/latonaio/rabbitmq-golang-client-for-data-platform"
)

type SupplyChainRelationshipExconfList struct {
	UIKeyGeneralUserID          string                                  `json:"ui_key_general_user_id"`
	UIKeyGeneralUserLanguage    string                                  `json:"ui_key_general_user_language"`
	UIKeyGeneralBusinessPartner string                                  `json:"ui_key_general_business_partner"`
	UIFunction                  string                                  `json:"ui_function"`
	UIKeyFunctionURL            string                                  `json:"ui_key_function_url"`
	RuntimeSessionID            string                                  `json:"runtime_session_id"`
	Params                      SupplyChainRelationshipExconfListParams `json:"Params"`
	ReqReceiveQueue             *string                                 `json:"responseReceiveQueue"`
}

type SupplyChainRelationshipExconfListParams struct {
	SupplyChainRelationship                  int     `json:"SupplyChainRelationship"`
	SupplyChainRelationshipID                *int    `json:"SupplyChainRelationshipID"`
	SupplyChainRelationshipBillingID         *int    `json:"SupplyChainRelationshipBillingID"`
	SupplyChainRelationshipPaymentID         *int    `json:"SupplyChainRelationshipPaymentID"`
	SupplyChainRelationshipProductionPlantID *int    `json:"SupplyChainRelationshipProductionPlantID"`
	SupplyChainRelationshipDeliveryPlantID   *int    `json:"SupplyChainRelationshipDeliveryPlantID"`
	BusinessPartner                          *int    `json:"BusinessPartner"`
	UserID                                   *string `json:"UserId"`
	User                                     *string `json:"User"`
	Buyer                                    *int    `json:"Buyer"`
	Seller                                   *int    `json:"Seller"`
	Payer                                    *int    `json:"Payer"`
	Payee                                    *int    `json:"Payee"`
	BillToParty                              *int    `json:"BillToParty"`
	BillFromParty                            *int    `json:"BillFromParty"`
	DeliverToParty                           *int    `json:"DeliverToParty"`
	DeliverFromParty                         *int    `json:"DeliverFromParty"`
	DeliverToPlant                           *string `json:"DeliverToPlant"`
	DeliverFromPlant                         *string `json:"DeliverFromPlant"`
	Language                                 *string `json:"Language"`
	IsCancelled                              *bool   `json:"IsCancelled"`
	IsMarkedForDeletion                      *bool   `json:"IsMarkedForDeletion"`
}

func ReadSupplyChainRelationshipExconfList(msg rabbitmq.RabbitmqMessage) *SupplyChainRelationshipExconfList {
	d := SupplyChainRelationshipExconfList{}
	err := json.Unmarshal(msg.Raw(), &d)
	if err != nil {
		fmt.Printf("%v\n", err)
		return nil
	}
	return &d
}
