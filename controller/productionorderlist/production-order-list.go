...
import (
	...
	"data-platform-api-request-reads-cache-manager-rmq-kube/api_requests/creator/productionorderlist"
...
type ProductionOrderListCtrl struct {
	cache *cache.Cache
...
func NewProductionOrderListCtrl(ctx context.Context, c *cache.Cache, rmq *rmqsessioncontroller.RMQSessionCtrl, log *logger.Logger) *ProductionOrderListCtrl {
	return &ProductionOrderListCtrl{
		cache: c,
		rmq:   rmq,
		ctx:   ctx,
		log:   log,
...
func (c *ProductionOrderListCtrl) ProductionOrderList(msg rabbitmq.RabbitmqMessage, l *logger.Logger) error {
	start := time.Now()
	params := extractProductionOrderListParam(msg)
	...
	if poRes.Message.Header == nil || len(*poRes.Message.Header) == 0 {
		c.finEmptyProcess(params, reqKey, "ProductionOrder", &cacheResult)
	...
	c.fin(params, poRes, bpRes, pRes, plantRes, reqKey, "ProductionOrder", &cacheResult)
	c.log.Info("Fin: %d ms\n", time.Since(start).Milliseconds())
	return nil
}

func (c *ProductionOrderListCtrl) productionOrderRequest(
	params *dpfm_api_input_reader.ProductionOrderListParams,
	sID string,
	reqKey string,
	...
	oiReq := productionorderlist.CreateProductionOrderReq(params, sID, c.log)
...
func (c *ProductionOrderListCtrl) businessPartnerRequest(
	params *dpfm_api_input_reader.ProductionOrderListParams,
	sID string,
	reqKey string,
	setFlag *RedisCacheApiName,
	...
	defer recovery(c.log)
	bpReq := productionorderlist.CreateBusinessPartnerReq(params, sID, c.log)
...
func (c *ProductionOrderListCtrl) plantRequest(
	params *dpfm_api_input_reader.ProductionOrderListParams,
	...
	defer recovery(c.log)
	dtpReq := productionorderlist.CreateDeliverToPlantReq(params, poRes, sID, c.log)
...
func (c *ProductionOrderListCtrl) productRequest(
	params *dpfm_api_input_reader.ProductionOrderListParams,
	poRes *apiresponses.ProductionOrderRes,
	sID string,
	reqKey string,
	...
	defer recovery(c.log)
	pmReq := productionorderlist.CreateProductRequest(params, poRes, sID, c.log)
...
func (c *ProductionOrderListCtrl) request(queue string, req interface{}, sID string, url, api string, setFlag *RedisCacheApiName) (rabbitmq.RabbitmqMessage, error) {
...
func extractProductionOrderListParam(msg rabbitmq.RabbitmqMessage) *dpfm_api_input_reader.ProductionOrderList {
	data := dpfm_api_input_reader.ReadProductionOrderList(msg)
	return data
}
...
func (c *ProductionOrderListCtrl) fin(
	params *dpfm_api_input_reader.ProductionOrderList,
	poRes *apiresponses.ProductionOrderRes,
	...
	for _, v := range *plantRes.Message.Generals {
		plants[v.Plant] = v.PlantName
	}

	productionOrders := make([]dpfm_api_output_formatter.ProductionOrder, 0)
	for _, v := range *poRes.Message.Header {
		productionOrders = append(productionOrders, dpfm_api_output_formatter.ProductionOrder{
			ProductionOrder:                     v.ProductionOrder,
			MRPArea:                             v.MRPArea,
			Product:                             v.Product,
			ProductName:                         pNames[*v.Product],
			OwnerProductionPlantBusinessPartner: bpRes.Message.General.BusinessPartnerName,
			OwnerProductionPlant:                plants[v.OwnerProductionPlant],
			...
			HeaderIsConfirmed:                   v.HeaderIsConfirmed,
			HeaderIsPartiallyConfirmed:          v.HeaderIsPartiallyConfirmed,
			HeaderIsReleased:                    v.HeaderIsReleased,
			// IsCancelled:                         nil,
			IsMarkedForDeletion: v.HeaderIsMarkedForDeletion,
		},
		)
	}

	data := dpfm_api_output_formatter.ProductionOrderList{
...
func (c *ProductionOrderListCtrl) finEmptyProcess(
	params interface{},
	url, api string, setFlag *RedisCacheApiName,
...
func (c *ProductionOrderListCtrl) Log(args ...interface{}) {
	for _, v := range args {
		b, _ := json.Marshal(v)
