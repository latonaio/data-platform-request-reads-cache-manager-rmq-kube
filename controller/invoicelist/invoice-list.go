...
func NewInvoiceListCtrl(ctx context.Context, c *cache.Cache, rmq *rmqsessioncontroller.RMQSessionCtrl, log *logger.Logger) *InvoiceListCtrl {
...
func (c *InvoiceListCtrl) InvoiceList(msg rabbitmq.RabbitmqMessage) error {
	// start := time.Now()
...
func (c *InvoiceListCtrl) invoiceRequest(
...
func (c *InvoiceListCtrl) businessPartnerRequest(
	params *dpfm_api_input_reader.InvoiceListParams,
...
func (c *InvoiceListCtrl) fin(
	...
	for _, v := range *iRes.Message.Header {
		invoices = append(invoices, dpfm_api_output_formatter.Invoices{
			...
			InvoiceDocumentTime:      v.InvoiceDocumentTime,
...
func (c *InvoiceListCtrl) request(queue string, req interface{}, sID string, url, api string, setFlag *RedisCacheApiName) (rabbitmq.RabbitmqMessage, error) {
...
func (c *InvoiceListCtrl) Log(args ...interface{}) {
	for _, v := range args {
		b, _ := json.Marshal(v)
